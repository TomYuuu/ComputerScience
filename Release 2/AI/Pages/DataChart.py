...
if selected_community:
    ...
    cols = st.columns(2)
    with cols[0]:
        st.markdown("### Transportation Metrics")
        st.metric("Public Transit Usage",
                  f"{safe_numeric(df[df['CommunityAreaName'] == selected_community]['PublicTransitUsage'].iloc[0]):.1f}%")
    with cols[1]:
        st.markdown("### Safety Metrics")
        st.metric("Crime Rate per 100k",
                  f"{safe_numeric(df[df['CommunityAreaName'] == selected_community]['CrimeRate'].iloc[0]):,.0f}")